...
import org.apache.spark.sql.types.{StructType, StructField, StringType, IntegerType, TimestampType, DoubleType, LongType}
...
object Main {
  ...
  val byVisitorOrderedByTimestamp = Window.partitionBy(REQUEST_IP).orderBy(TIMESTAMP)
  ...
  def main(args: Array[String]): Unit = {
    ...
    if(args.length == 2) {
      ...
      val path = args(0)
      val session_max_idle_time = args(1)
      ...
      elb_logs = elb_logs.filter(!(elb_logs(PROTOCOL)===""))

      elb_logs = elb_logs.cache()

      val elb_logs_sessionized = elb_logs.withColumn(SESSION_ID,
        concat_ws(
          "_",
          col(REQUEST_IP),
          sum(
            when(((col(TIMESTAMP) - lag(col(TIMESTAMP), 1).over(byVisitorOrderedByTimestamp)) >= session_max_idle_time), 1)
              .otherwise(0)
          ).over(byVisitorOrderedByTimestamp)))

      val session_times = elb_logs_sessionized
        .groupBy(REQUEST_IP, SESSION_ID)
      ...
      println("*"*25 + "AVERAGE SESSION LENGTH:" + "*"*25)
      avg_session_length.show(false)

      val unique_url_count = elb_logs_sessionized.groupBy(SESSION_ID).agg(countDistinct(URL).alias(UNIQUE_URL_COUNT))

      val avg_unique_url_count = unique_url_count.groupBy().agg(round(avg(UNIQUE_URL_COUNT), 2).alias(AVG_UNIQUE_URL_COUNT))
      println("*"*25 + "AVERAGE UNIQUE URL COUNT:" + "*"*25)
      ...
      val most_engaged_users = longest_sessions.select(REQUEST_IP).distinct()
